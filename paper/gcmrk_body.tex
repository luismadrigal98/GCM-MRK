...
on an explicit likelihood footing: a module is a block of the correlation matrix
with a $q$-factor structure and free loadings, and the absolute-correlation
criterion used throughout the field is the constrained special case in which
$q=1$ and all loadings share a magnitude. Making that nesting visible turns an
implicit modelling choice---that a module's members respond to their regulator
equally strongly, and to one regulator only---into a testable assumption with a
strictly more general alternative. Second, we hybridize the GA with an efficient
memetic local search and benchmark it not only against the agglomerative
clustering at the core of standard co-expression pipelines but against the
...
WGCNA~\cite{Langfelder2008} itself---each given the true number of modules or
tuned against ground truth, so that no margin can be attributed to unfavourable
settings for a competitor. Third, and unusually for a methods paper, we
characterize when our own objective is wrong: across a family of data-generating
processes that violate the block model we identify two structural regimes in
which maximizing the \emph{constrained} likelihood moves \emph{away} from the true
modules, demonstrate that this is model misspecification rather than optimizer
failure, and show that the free-loading model removes the failure. Fourth, we
show the factor rank can be selected from the data by BIC, and---because a
negative result here is as useful as a positive one---that the same criterion
cannot select the number of modules, for a reason we make explicit; the
constrained criterion remains the right tool for that. The implementation is
deliberately minimal---depending only on NumPy~\cite{Harris2020} and
SciPy~\cite{Virtanen2020}, with internal k-means++~\cite{ArthurVassilvitskii2007}
and correlation-distance hierarchical seeders, behind one swappable-metric
interface that also exposes standard geometric indices and a multi-objective
(NSGA-II~\cite{Deb2002}) mode. We evaluate \tool{} on synthetic data across a
replicated noise sweep, on misspecified generators, on the classic Iris
benchmark~\cite{Fisher1936}, and on a breast-cancer RNA-seq dataset.
...
Because $\mathcal{L}$ (Eq~\eqref{eq:loglik}) has no parsimony term (penalization against more clusters), \tool{} offers three routes to the right number of clusters (a fourth, based on the factor block model below, is described in that section). (i) \emph{Constrain} $g_{\max}$ to the expected number of modules. (ii) \emph{Optimize a penalized correlation criterion} (\texttt{loglik\_aic} or \texttt{loglik\_bic},
...
answer to more than one regulator. Relaxing both gives the model we recommend by
default, of which Eq~\eqref{eq:loglik} is a constrained special case.
...
\begin{equation}
\mathbf{R}_m \;=\; \mathbf{\Lambda}_m\mathbf{\Lambda}_m^{\!\top} + \sigma_m^2\mathbf{I},
\qquad \mathbf{\Lambda}_m \in \mathbb{R}^{n_m \times q},
...
member of the family: it costs roughly a third to a quarter as much per
evaluation and requires no rank selection, at the price of the two assumptions
above. We report both throughout, and quantify what the constraint costs.
...
We use the adjusted Rand index (ARI)~\cite{HubertArabie1985}; for the single-dataset summaries (Table~\ref{tab:benchmark}) we also report Hungarian-matched accuracy. Results are reported as mean $\pm$ SD over the random datasets. All code to reproduce the experiments, figures, and tables is provided with the package.
...
that it does at every noise level tested, though the size of the margin and which
variant delivers it both depend on the noise.

Driven by the factor objective with the rank selected by BIC, \tool{} attains the
best recovery of any method at both informative noise levels: ARI
$\FactorAutoHardARI{}$ at $\sigma=1.5$ and $\FactorAutoExtremeARI{}$ at
$\sigma=2.0$, against $\WgcnaOraHardARI{}$ and $\WgcnaOraExtremeARI{}$ for
oracle-tuned WGCNA, the strongest competitor. At lower noise every method except
k-means and MCL saturates near ARI $1.0$ and the comparison is uninformative.
Under the constrained objective of Eq~\eqref{eq:loglik}, \tool{} reaches
$\MemeticHardARI{}$ at $\sigma=1.5$---still ahead of every competitor, and
significantly so under a paired Wilcoxon signed-rank test across the \NSeeds{}
shared datasets ($p<0.001$ against each)---but falls to $\MemeticExtremeARI{}$ at
$\sigma=2.0$, where oracle-tuned WGCNA overtakes it (paired difference $-0.11$,
$p=1\times10^{-4}$).

That contrast is itself informative. The equal-loading constraint costs little
when the signal is strong and a great deal when it is weak: at high noise the
effective loadings of a module's members are most heterogeneous, and it is
exactly there that freeing them recovers a lead. This is the first of several
results in which the constrained model's failures are concentrated where its
assumption is least tenable, rather than distributed at random.

Two further points belong with these numbers. First, the margin over the best
competitor is modest---$0.04$ ARI at $\sigma=1.5$---where the margin over
average-linkage hierarchical clustering, the comparison a reader of the earlier
GA-clustering literature would expect, is $0.15$. A benchmark restricted to
...
Second, the ablation isolating the memetic local search is noise-dependent in the
same way: at $\sigma=1.5$ disabling it costs $0.065$ ARI
($p=2\times10^{-4}$), confirming that the hill-climb---which reassigns genes
\emph{a posteriori}, a move greedy agglomeration cannot make---is responsible for
much of the margin, while at $\sigma=2.0$ the difference is not significant. Local
search is decisive where the likelihood surface is rugged but still informative,
not universally.
...
agglomerative clustering does, and oracle-tuned WGCNA overtakes it at
$\sigma=2.0$.}
\label{fig:perfnoise}
\end{figure}
...
factor objective leads on all \MisspecTotal{}: its own one-factor design (ARI
$\MisspecOneFacFactor{}$ against $\MisspecOneFacBest{}$ for the best competitor),
overlapping modules ($\MisspecOverlapFactor{}$ versus $\MisspecOverlapBest{}$),
negative-binomial counts ($\MisspecCountsFactor{}$ versus
$\MisspecCountsBest{}$), network-derived correlation ($\MisspecNetworkFactor{}$
versus $\MisspecNetworkBest{}$), multi-factor modules
($\MisspecMultiFacFactor{}$ versus $\MisspecMultiFacBest{}$) and hub-and-spoke
modules ($\MisspecHubFactor{}$ versus $\MisspecHubBest{}$). The rank is selected
without supervision in each case, and recovers the generating structure: BIC
chooses $q=\MisspecMultiFacRank{}$ on the three-factor process and
$q=\MisspecOneFacRank{}$ on the rest.

The constrained objective of Eq~\eqref{eq:loglik} behaves quite differently, and
the pattern of where it fails is the substantive result. It leads on
\MisspecWins{} of the \MisspecTotal{} processes and fails on two. Where modules
are driven by a single dominant signal it is competitive or better---one-factor
$\MisspecOneFacGCM{}$, overlapping $\MisspecOverlapGCM{}$, counts
$\MisspecCountsGCM{}$, network $\MisspecNetworkGCM{}$---and the counts result is
worth emphasizing, because replacing the Gaussian with a gamma--Poisson mixture
and adding a mean--variance trend does not degrade it. \emph{Distributional}
misspecification is not what hurts this model.

\emph{Structural} misspecification is. On multi-factor modules---where a module
spans a three-dimensional subspace rather than a single direction---the
constrained objective recovers ARI $\MisspecMultiFacGCM{}$ where the best
competitor reaches $\MisspecMultiFacBest{}$, and on hub-and-spoke modules, whose
members carry systematically unequal loadings, $\MisspecHubGCM{}$ against
$\MisspecHubBest{}$. Both are exactly the regimes in which the equal-magnitude,
single-factor constraint is false, and in both the free-loading model removes the
deficit entirely. The failures are therefore attributable to a specific,
identifiable assumption rather than to the framework.
...
scores \emph{higher} in every replicate (for example $16.8$ against $15.4$ at
$\sigma=1.0$), while its ARI against the truth is near $0.5$. The optimizer is
...
\subsection*{Freeing the loadings removes the failure}
...
It does, and by more than repair. With the rank selected from the data by BIC
(\texttt{cluster\_factor\_auto}; Methods), multi-factor recovery rises from
...
$q=\MisspecOneFacRank{}$ on the one-factor, hub, counts and network processes, and
the automatically selected rank attains the same recovery as the best rank chosen
with knowledge of the truth. The extra flexibility therefore costs computation but
not accuracy.
...
the one-factor design recovery falls from $1.00$ to $0.58$, because the surplus
...
recommend BIC over AIC here, since AIC systematically selects one rank too many.

The factor branch is offered alongside the exchangeable objective rather than as
a replacement. It costs roughly three to four times more per run, multiplied by
the number of candidate ranks, and on data that genuinely follow the one-factor
model it offers nothing over Eq~\eqref{eq:loglik}. Its value is that a user who
does not know which regime their data occupy can let BIC decide.
...
margin, which is a few hundredths of ARI, these are differences of $0.25$ to
$0.54$.
...
three to four that its per-evaluation cost implies, but by substantially more
once the data support many modules. On the empirical dataset of the next section
the constrained criterion fitted $300$ genes in $14$~s where the two-stage factor
procedure took $463$~s, because the local search must re-solve an
eigendecomposition per affected block and real correlation structure yields both
more modules and a higher selected rank than the synthetic designs do. The factor
model is therefore the better choice on gene sets of hundreds to low thousands of
genes, and the constrained objective the practical one above that; users needing
both can fit the factor model on a variance-filtered subset and extend the
resulting modules with the cheaper criterion.
...
GSE183947~\cite{Wang2021}, a breast-cancer RNA-seq dataset of \EmpSamples{}
...
(Benjamini--Hochberg $q<0.05$), which is unsurprising given that the
...
\emph{not} outperform the constrained objective here, on any of the four criteria,
despite leading on every synthetic process. With one dataset, \EmpSamples{}
samples and differences this small we cannot say whether that reflects a genuine
property of these data---modules that really are single-signal, where the extra
flexibility only adds variance---or simply the limits of what this dataset can
resolve. It does mean the synthetic advantage should not be assumed to transfer
automatically to real expression data, and we do not claim that it does. Second,
...
loadings \cite{bartholomew2011} (\nameref{S1_Appendix}). Writing it that way makes
two constraints visible---one factor, equal magnitudes---and both can be relaxed
within the same family. Doing so is what our results recommend: the free-loading
factor model of Eq~\eqref{eq:factorblock} is at least as good as the constrained
form on every process and every noise level we tested, and strictly better where
the constraints bind. We therefore treat the factor model as the default and the
absolute-correlation criterion as a fast, strongly constrained approximation to
it---cheaper, requiring no rank selection, and adequate whenever modules really
are driven by one regulator whose members respond comparably.

That recommendation carries an important qualification. On the one empirical
dataset we analyse, the factor model does not outperform the constrained form on
any criterion we could apply, and it is markedly more expensive there than its
per-evaluation cost on synthetic data suggests. We therefore state the
recommendation exactly as far as our evidence supports it: the factor model is
the better choice when there is reason to think module members respond at
different strengths or to several regulators, the constrained form is a sensible
default otherwise, and on real data of this size we could not distinguish them.
Which regime real co-expression data typically occupy is an open question that
larger cohorts would settle.
...
at $\sigma=2.0$ that competitor overtakes us. A benchmark restricted to
hierarchical clustering and k-means---the comparison the earlier literature on
GA-based clustering has largely used---would have overstated the advantage by
roughly fourfold. Where the margin is unambiguous is in the presence of
unstructured genes, a situation any variance-filtered gene set presents, and we
regard that as the more consequential practical result.
...
with the rank selected from the data it leads every process we tested, including
the two the exchangeable objective fails on, and it is the only variant that
beats oracle-tuned WGCNA at the highest noise level. Because its parameter count
...
when at least one of them ranks the truth highly, and we found that graph
modularity, the natural second objective, is itself misspecified on the same
processes. Search strategy cannot repair a wrongly located optimum. As a
...
with a memetic genetic algorithm. Treating a module as a correlation block with a
$q$-factor structure and free loadings, and recovering the field's usual
absolute-correlation criterion as the constrained $q=1$ equal-magnitude special
case, puts both on one footing and makes the choice between them empirical rather
than conventional. On that footing the free-loading model is the better default:
it attains the best recovery of any method we tested at every informative noise
level, leads on all six data-generating processes including the two where the
constrained form fails outright, and selects its own rank from the data. The
constrained criterion remains valuable as a fast approximation, and---because its
penalty grows with the module count where the factor model's does not---it
remains the appropriate tool for choosing how many modules there are.

Making the generative model explicit also let us test it, and we regard the
resulting negative results as part of the contribution rather than a caveat to
it: we identify two structural regimes in which the constrained objective is
misspecified, demonstrate that its optimizer there finds partitions scoring above
the ground truth, and show that the factor model removes the failure; and we show
that the criterion which selects the rank so reliably cannot select the number of
modules, for a reason we make explicit. \tool{}'s largest and most robust
practical advantage---isolating genes that belong to no module---is likewise a
consequence of the model rather than of the search. The same engine generalizes
to geometric indices and recovers phenotype-associated modules in breast-cancer
RNA-seq. We suggest that reporting where a method's objective ceases to be
appropriate, alongside where it succeeds, is more useful to practitioners than a
benchmark in which it always wins. The software, documentation, and full
reproduction scripts are openly available.
