...
on an explicit likelihood footing: the absolute-correlation criterion used
...
for a competitor. Third, we test on real data with real ground truth, using the
benchmark of Saelens et al.~\cite{Saelens2018}, which pairs expression compendia
from three organisms with curated module definitions derived from regulatory
networks. Fourth, and unusually for a methods paper, we characterize when our own
...
model removes the failure on those processes---while reporting that this
advantage does not carry over to any of the real datasets we tested, which is a
result about the structure of real co-expression data rather than about the
optimizer. The implementation is deliberately minimal---depending only on
NumPy~\cite{Harris2020} and SciPy~\cite{Virtanen2020}, with internal
...
larger, between twenty and a hundred times (Results), because real correlation
...
modules that biologists have independently curated. We therefore evaluated on the
benchmark of Saelens et al.~\cite{Saelens2018}, which pairs real expression
...
transfer. It is worse than the constrained criterion on \emph{all}
\SaelensFactorBeaten{} real datasets (sign test $p=\SaelensFactorP{}$; mean F1
...
between twenty and a hundred times more than the constrained criterion at this
...
is low throughout---from $0.02$ on human to $0.31$ on \emph{E. coli}---which
...
\begin{adjustwidth}{-1.25in}{0in}
\centering
\IfFileExists{results/saelens_table.tex}{\input{results/saelens_table.tex}}{%
\begin{tabular}{@{}l@{}}\hline
...
three to four that its per-evaluation cost implies, but by between twenty and a
hundred times on real data. On the Saelens compendia the constrained criterion
fitted $\SaelensNTop{}$ genes in 28 to 165~s where the two-stage factor procedure
took 1836 to 4064~s, because the local search must re-solve an eigendecomposition
...
tested, never wins one, and costs twenty to a hundred times more. We therefore
...
very different strengths or to several regulators---a situation we can construct
...
repairs the failure completely on those processes. We then report that this
