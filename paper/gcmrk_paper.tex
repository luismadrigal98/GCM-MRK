...
something else and only measure correlation afterwards. We present a genetic clustering machine (\tool{}), an open-source Python tool that closes this gap by treating module detection as maximum-likelihood inference and solving it globally. We show that the project's correlation objective is, up to a constant and the sample-size factor, the
...
replicated noise sweep (Fig~\ref{fig:perfnoise}, Table~\ref{tab:perf}) shows that the answer is yes, decisively and reproducibly. At the hard noise level ($\sigma=1.5$, mean
...
hill-climb with the designed reassignment of elements \textit{a posteriori} (a move greedy agglomeration cannot make) the feature that produces the decisive margin and the low variance.
...
This footing makes its model selection principled, in the sense that the number of modules is chosen by a genuine BIC/AIC over the $d$ samples (Eq~\eqref{eq:loglikic}), and
...
pipelines rely. This advantage we establish with a replicated benchmark and an
