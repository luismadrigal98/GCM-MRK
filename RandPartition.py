...
class RandPartition:
    ...
    def __init__(self, gene_number, G=2, all_in_clusters=True):
        """
        Initialize the instance variables and create a random partition.
        """

        self.gene_number = gene_number
        self.__init_part(gene_number, G, all_in_clusters)
    ...
    def __init_part(self, gene_number, G, all_in_clusters=True):
        """
        Create a random partition of genes into clusters.
        """

        start = 1 if all_in_clusters else 0
        self.items = [random.randint(start, G) for _ in range(gene_number)]

    def __get_values_int(self, Data, DBI = True, BIC = True, AIC = True):
        """
